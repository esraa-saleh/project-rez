...
class Collector:
    ...
    def collectFullRun(self, name, run_data):
        self.run_data[name] = run_data
        self.reset()
    ...
    def collectBatch(self, name, batch):
        arr = self.run_data.get(name, [])
        arr.extend(batch)

        self.run_data[name] = arr

    def getCurrentRunData(self, name):
        return self.run_data[name]

    def getAllRuns(self, name):
